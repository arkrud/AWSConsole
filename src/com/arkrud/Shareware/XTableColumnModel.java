...
/**
 * <code>XTableColumnModel</code> extends the DefaultTableColumnModel . It provides a comfortable way to hide/show columns. Columns keep their positions when hidden and shown again.
 * 
 * In order to work with JTable it cannot add any events to <code>TableColumnModelListener</code>. Therefore hiding a column will result in <code>columnRemoved</code> event and showing it again will
 * notify listeners of a <code>columnAdded</code>, and possibly a <code>columnMoved</code> event. For the same reason the following methods still deal with visible columns only: getColumnCount(),
 * getColumns(), getColumnIndex(), getColumn() There are overloaded versions of these methods that take a parameter <code>onlyVisible</code> which let's you specify weather you want invisible columns
 * taken into account.
 * 
...
public class XTableColumnModel extends DefaultTableColumnModel {
	...
	/**
	 * Maps the index of the column in the table model at <code>modelColumnIndex</code> to the TableColumn object. There may me multiple TableColumn objects showing the same model column, though this is
	 * uncommon. This method will always return the first visible or else the first invisible column with the specified index.
	 * 
	...
	/**
	 * Append <code>column</code> to the right of exisiting columns. Posts <code>columnAdded</code> event.
	 * 
	 ...
	 * @exception IllegalArgumentException if either <code>oldIndex</code> or <code>newIndex</code> are not in [0, getColumnCount() - 1]
	 */
	...
	/**
	 * Returns the position of the first column whose identifier equals <code>identifier</code>. Position is the the index in all visible columns if <code>onlyVisible</code> is true or else the index in
	 * all columns.
	 * 
	 ...
	 * 
	 * @exception IllegalArgumentException if <code>identifier</code> is <code>null</code>, or if no <code>TableColumn</code> has this <code>identifier</code>
	 * @see #getColumn
	 */
